...
## 7. v2.1 — churn-engine corrections (2026-09-03, scoopy's critique)

Three structural fixes after scoopy flagged the sim underestimating 28d-redemption churn, mis-modeling LP depth, and hiding the deposit pulse:

**1. Standing float capture (the churn engine).** Previously redemptions only happened when new-mint flow pushed through resting bids — float piled to $341M doing nothing. Now: whenever the discount implies arb-APR ≥ `float_capture_apr` (5.5%), arb desks continuously buy float and queue it for the 28d vest, capital-bound (`room = arb × deploy/2` — each buy locks capital until payout). Churn is therefore a function of defender capital supply, which is the honest peg-defense equilibrium. Result at BASE: float $341M → $27.7M, churn 1.7 → ~5.0 cycles/yr, peg pins at T-bill parity (0.9958-0.9962), arb ends mostly LOCKED in vesting (free arb $3M against ~$200M through-flow).

**2. LP depth = f(volume).** Fees now accrue on ALL swap legs (mint sells + standing float buys + exit re-buys) — pool volume $984M/2yr at BASE vs the old mint-only ~$1.18B... (and now feeds depth). Depth relaxes at `lp_track`/day toward the income-implied target `(fees×365 + w×emissions×365) / lp_alt_apr` (BE's depth = LP income / r_lp); emissions weighted 0.35 (mercenary capital). Depth $3.7M → $9.2M at BASE; the thin-launch phase now shows the depth ramp as volume arrives.

**3. Deposit pulse.** At redemption maturity a fraction (`equity_exit_frac` 5%) of freed looper equity exits (profit-take) instead of silently re-looping — the shrink half of the pulse; the spike half is deposit inflow chasing looper APR. New DEPOSIT PULSE chart: net equity flow (blue, ~$64k/d) vs redemption payouts (red, ~$942k/d) vs arb float buys (amber, ~$276k/d) — payout waves visibly dominate and oscillate (106 directional turns in d100-400). Steady state is smooth-ish by construction (linear vest spreads waves); pulses concentrate at launch and around shocks — as they should.

**Systemic consequence:** the churn engine BLEEDS growth. Every cycle costs the 10bp debtor fee plus the equity exit, so BASE settles at C=$133M/D=$116M (vs $528M/$468M pre-fix) despite identical inflows. DAO revenue mix shifts materially: perf $803k + redemption $387k — the protocolFee is now ~48% of perf revenue (was ~17%) because redemption volume runs ~5x the old model. The redemption fee became a first-class revenue instrument; its Laffer economics deserve the same scrutiny as the perf fee.
